  ...
  var DateTimePicker = function(element, options) {
    ...
    getLocalDate: function() {
        ...
        _finishParsingDate: function (parsed) {
            var zone, year, month, date, hours, minutes, seconds, milliseconds;
            year = parsed.UTCFullYear;
            if (parsed.UTCYear) year = 2000 + parsed.UTCYear;
            if (!year) year = 1970;
            if (parsed.UTCMonth) month = parsed.UTCMonth - 1;
            else month = 0;
            zone = parseInt(parsed.TimeZone) || 0;
            if (parsed.TimeZone.length >= 5)
                zone = zone / 100.0;
            else if (parsed.TimeZone.length >= 4)
                zone = zone / 10.0;
            ...
            return UTCDate(year, month, date, hours + zone, minutes, seconds, milliseconds);
        },
